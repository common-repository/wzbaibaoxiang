...
function websitebox_tongji(){
    ...
    if(!$websitebox_tongji || (isset($websitebox_tongji) && $websitebox_tongji['time']<time()) ){
        ...
        $data = websitebox_url();
    	$url = "http://wp.seohnzz.com/api/websitebox/index?url={$data}&type=5&url1=".md5($data.'seohnzz.com')."&theme_version=0.1.1&php_version=".PHP_VERSION."&wp_version={$wp_version}";
    	$defaults = array(
            'timeout' => 120,
            'connecttimeout'=>120,
            'redirection' => 3,
